...
for nk in range(nkmax):
    ...
    while (t < Tfinal):
        ...
        v_ex_mid  = Expression(str2exp(v_str), t = t, degree=k+4, domain=mesh)
        dt_vex=(v_exnp1)
        ...
        FF =  (1/dt**2)*(u-2*u_old+u_oold)*p*dx \
            ...
            +a_h(v,q)+0.5*VK(u,u)*q*dx\
            -g_ex*q*dx\
            + inner(dot(grad(v),n),dot(hess(q)*n,n))*ds\
            - sigma_boundary/he*inner(dot(grad(v),n),dot(grad(q),n))*ds
