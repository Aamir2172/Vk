...
Tfinal = 2; 
dtvec = [1./2,1./4, 1./8, 1./16, 1./32, 1./64,1./128] 
nkmax = 6
...
for nk in range(nkmax):
    ...
    while (t < Tfinal):
        ...
        v_exmid  = 0.5*(v_exnp1+v_exn)#Expression(str2exp(v_str), t = t+dt, degree=k+4, domain=mesh)
        ...
        g_hat = 0.25*(div(grad(div(grad(v_exnp1))))+2*div(grad(div(grad(v_exn))))+div(grad(div(grad(v_exnm1))))+0.5*VK(u_exnp1,u_exnp1)+VK(u_exn,u_exn)+0.5*VK(u_exnm1,u_exnm1))

        g_bar = 0.5*(div(grad(div(grad(v_exnp1))))+div(grad(div(grad(v_exn))))+0.5*VK(u_exnp1,u_exnp1)+0.5*VK(u_exn,u_exn))

        g_ex = div(grad(div(grad(v_exnp1))))+0.5*VK(u_exnp1,u_exnp1)
        ...
        FF =  (1/dt**2)*(u-2*u_old+u_oold)*p*dx \
            ...
            - g_bar*q*dx\
            + inner(dot(grad(v_exbar),n),dot(hess(q)*n,n))*ds\
            - sigma_boundary/he*inner(dot(grad(v_exbar),n),dot(grad(q),n))*ds
        ...
        Ev_sdiv= max(Ev_sdiv,energynorm(v_exmid-v_mid))
